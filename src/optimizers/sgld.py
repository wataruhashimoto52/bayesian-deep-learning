...
class pSGLDV2(Optimizer):
    ...
    def __init__(self, params, lr=np.float(1e-3), alpha=0.99, eps=1e-8, centered=False, addnoise=True):
        defaults = dict(lr=lr, alpha=alpha, eps=eps,
                        centered=centered, addnoise=addnoise)
        super(pSGLDV2, self).__init__(params, defaults)

    def __setstate__(self, state):
        super(pSGLDV2, self).__setstate__(state)
        for group in self.param_groups:
