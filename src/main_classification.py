...
def train(model, device, train_loader, optimizer, scheduler, epoch, args, config, logger):
    ...
    total_correct = 0
    loss = None
    confidence = {'top1': 0, 'top1_true': 0,
                  'top1_false': 0, 'true': 0, 'false': 0}
    total_data_size = 0
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if isinstance(optimizer, SecondOrderOptimizer) and optimizer.curv_type == "Fisher":
            closure = torchsso.get_closure_for_fisher(
                optimizer, model, data, target, **config["fisher_args"])
        ...
        if batch_idx % args.log_interval == 0:
            ...
            print('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}, '
                  ...
                  'Elapsed Time: {:.1f}s'.format(
                      epoch, total_data_size, epoch_size, 100. *
                      (batch_idx + 1) / num_iters_in_epoch,
...
def main():
    ...
    parser.add_argument('--dataset', type=str,
                        choices=[DATASET_CIFAR10, DATASET_CIFAR100,
                                 DATASET_MNIST], default=DATASET_CIFAR10,
    ...
    train_dataset = dataset_class(
        root=args.dataroot, train=True, download=args.download, transform=train_transform)
    val_dataset = dataset_class(
        root=args.dataroot, train=False, download=args.download, transform=val_transform)
